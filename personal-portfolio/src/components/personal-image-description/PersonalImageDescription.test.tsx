...
describe("PersonalImageDescription", () => {
    it("renders the header, subheader, and bio", () => {
        ...
        expect(screen.getByText(/I'm a full-stack developer at/)).toBeInTheDocument();
    });

    it("renders the company link with correct href", () => {
        render(<PersonalImageDescription />);
        const link = screen.getByRole("link", { name: "Insight Research Ireland Centre for Data Analytics" });
        expect(link).toHaveAttribute("href", "https://www.insight-centre.org/");
